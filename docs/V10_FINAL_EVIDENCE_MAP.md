...
| Accepted product identity | exact release probe; `V10_RELEASE_STATUS.md` | Product and public `main` commit `05fe9d1…`; immutable deployment `dpl_GZhotq…`. |
...
| Release test gate passed | current release transcript; `V10_RELEASE_STATUS.md` | Hardened tree: 84 unit files / 1,393 tests, typecheck and 125-module production build. Exact stable and immutable deployment: web 9/9 and demo/API 30/30; stable Google boundary 16/16. The earlier 198/198 normal plus 198/198 reduced-motion audit remains structural route evidence; final sign-in and first-use copy were browser-checked after promotion. |
...
| Google OAuth security boundary passed | current `npm run smoke:google`; `scripts/smoke-google-auth.ts` | 16/16 through the real account chooser, including exact state/challenge binding. Human identity selection and fresh callback were intentionally not completed. |
