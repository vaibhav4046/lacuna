      ...
      <div style={{ position: 'sticky', top: 0, height: '100vh', overflow: 'hidden' }}>
        <div data-reveal style={{ position: 'absolute', top: '50%', translate: '0 -50%', left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '24px', textAlign: 'center', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '26px', marginTop: '4px' }}>
            {account.state === 'pending' ? null : (
              <button className="hv-violet" onClick={() => go(account.primary.path)} style={{ background: '#8052FF', border: 'none', cursor: 'pointer', color: '#FFFFFF', fontSize: '15px', fontWeight: 500, padding: '13px 24px', borderRadius: '8px', whiteSpace: 'nowrap' }}>{account.state === 'member' ? account.primary.label : 'Start building'}</button>
            )}
