...
| exact candidate preview | `dpl_6kGLHtEZYK2Uh1QuiSeCnAS8q1Z9` is READY from commit `cb0e9f4` at `https://lacuna-4ab1x4h2p-vaibhav4046s-projects.vercel.app`; basic auth smoke passes 3/3, but Google smoke is 5/16 because preview has no production Google credentials, so it was not promoted | Vercel deployment API and `npm run smoke:auth`, `npm run smoke:google` on 2026-08-22 |
...
| focused connector, auth, voice, agent and onboarding gates | focused connector/auth/voice/agent regressions plus onboarding readiness coverage passed; current candidate full unit suite is 2,294/2,294 | serial terminal runs on 2026-08-22 with one worker |
