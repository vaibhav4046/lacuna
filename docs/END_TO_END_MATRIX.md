...
| Unit tests | The suite that needs no database | `npx vitest run tests/unit` | 1002 passed, 48 files | [RELEASE_GATE.md](../RELEASE_GATE.md). The committed artifact [unit.txt](../artifacts/verification/2026-08-18-gates/unit.txt) records the earlier count of 921 over 41 files and is kept unedited |
...
| Mobile and reduced motion | Nine viewports from 360x800 to 3840x2160, then the same sweep with prefers-reduced-motion set | `npm run audit:routes`, then with `--reduced-motion` | 207 of 207 in each pass, 0 scrolling sideways | [artifacts/route-audit/routes.json](../artifacts/route-audit/routes.json), [routes-reduced-motion.json](../artifacts/route-audit/routes-reduced-motion.json) |
