...
| Google callback stall/parameter guard (queued) | release candidate rejects malformed or oversized state/code before provider contact; token/JWKS exchange has a measured ten-second deadline and returns a fixed timeout message instead of freezing the sign-in form; preview is built but not production-promoted because the Vercel quota is exhausted | `src/api/router.ts`, `src/auth/google.ts`, `web/src/auth/google-problem.ts`, `tests/unit/google-auth-api.test.ts`, preview `dpl_5gZuGA173TBvQEsupCgR4d4evfF8` |
