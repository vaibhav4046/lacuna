...
### D-091: the React product is a sibling under `web/`, not a fork

The frozen design is a single page application with a canvas engine and live
audio in it. The shipped site is server rendered with zero client JavaScript.
Those are two different delivery models for one product, and the temptation
was to branch: keep the server rendered site, build the SPA somewhere else,
reconcile later. That is how a codebase ends up with two answers to the same
question, which is the one thing this product is about not doing.

So `web/` is a sibling application in this repository, and the rule is one
core. It talks to the same server over `/api`, and the Vite dev server proxies
to the real Node process on 3014 rather than to a mock, because a mock is a
second implementation of the answer path wearing a disguise. The server
rendered site stays live at its URL until the React build passes every gate,
then it is cut over in one commit and the old view layer is deleted.

Ports: 3014 is the existing server, 3015 the snapshot and parity harnesses,
3016 the React dev server, so all three can run while a route is checked
against the oracle.

### D-092: one `/app/:route` parameter route, not eighteen siblings

The design renders one application shell and switches an `sc-if` per route
inside it. Eighteen sibling `<Route>` elements would remount that shell on
every navigation, which is a different behaviour dressed as the same one: the
sidebar, the header and anything the shell holds would tear down and rebuild
between clicks.

One parameter route keeps the shell mounted and moves the decision into
`isRouteKey`, which is checked against the design's own `TITLES` map. An
unknown key redirects to the dashboard instead of rendering an empty shell.
The nav array carries the same guarantee at build time: `NAV_GROUPS` is
declared `satisfies` a type whose second tuple element is `RouteKey`, so a
sidebar entry naming a route that does not exist stops the build rather than
shipping a dead button.

### D-093: the design's runtime is reimplemented, not ported

`Lacuna Product.dc.html` loads a 69KB `support.js` that implements a small
template DSL: `sc-if`, `sc-for`, brace interpolation, `style-hover`
attributes, and a React-shaped lifecycle on a logic class. None of that is
portable and none of it needs to be. `sc-if` is a conditional render, `sc-for`
is a `.map`, a template hole is a prop, the logic class is a component with
`useEffect` and refs. The DSL disappears; the rendered output does not change.

The one thing that survives verbatim is the markup and its inline `style`
values. React assigns inline styles through the CSSOM rather than emitting a
`style` attribute, so the strict content security policy this application will
ship with never needs `unsafe-inline` for them. Porting the design literally
turns out to be the option that also keeps the policy tight.

### D-094: the icon set was extracted by script, not retyped

`ICOS()` builds sixteen icons as `data:image/svg+xml` URIs from long path
strings. Retyping sixteen SVG paths is exactly how a claim of "ported
verbatim" quietly becomes false, so `web/src/design/icons.ts` was generated by
piping lines 1367 to 1382 of the design between a header and a tail. The bytes
in the file are the bytes in the design.

Two consequences worth naming. Nothing needs to be copied out of
`design/incoming2/assets/`, because no icon is a file. And every icon is
covered by `img-src 'self' data:` without loosening anything, because no icon
is a request either.

The `claude` entry stays. It is a model provider the product connects to, the
same as `codex` or `gitlab`, and removing it would be editing the design's
product surface to satisfy a rule about authorship attribution that has
nothing to do with it.

### D-095: nine font files, twenty-one declarations, no CDN

The design links Google Fonts. The product must not, so the faces are self
hosted. Google serves Space Grotesk and JetBrains Mono as variable fonts, one
file per unicode subset rather than one per weight, so the three Space Grotesk
weights share three files and the two JetBrains Mono weights share six. Nine
files back all twenty-one `@font-face` declarations.

All nine subsets ship, not just latin. `unicode-range` means a browser never
downloads a subset it has no glyphs for, so the extra files cost nothing at
runtime, and the requirement that rendered output be identical then holds for
text a user typed as well as for the design's own copy.

### D-096: one stylesheet, and what is allowed in it

Pseudo-states cannot be inline styles, so exactly one stylesheet exists. It
holds the design's own head style block verbatim, the `@font-face`
declarations, and one class per distinct `style-hover` value. There are six
distinct hover values across twenty-eight uses in the design and one focus
value across eight, so there are seven classes. Nothing else moved into CSS:
no layout, no colour, no spacing, no token refactor.

One rule was added rather than ported. The design pairs `outline:none` with a
violet border on the eight inputs that have a focus state, and gives buttons
and links no focus treatment at all. Keyboard focus has to be visible, so
`:focus-visible` draws a two pixel ring in the design's own `#8052FF`. It is
keyboard only, so a mouse user sees exactly what the design shows.

### D-097: the design tokens file and the asset directory are not the design

The tokens file under `design/incoming2/uploads/` opens with a comment calling
itself immutable, and then encodes a different product: an orange `#ff5719`,
Geist instead of Space Grotesk, a collapsing rail. It is an earlier
generation. The brief is explicit that `Lacuna Product.dc.html` is the only
source of truth, so the tokens file is rejected.

Its contamination reached `design/incoming2/assets/favicon.svg`, whose
terminal dot was that same orange and whose spiral had two arcs where the
design's `SP()` has three radii. That file had already been copied into
`web/public/` on the reasoning that it sat beside the design. Reading the
bytes back disproved it. The favicon is now written from the design's own
inline mark: three arcs, amber `#FFB829`, zero occurrences of the orange.

The standing rule: verify every colour in an asset against the inline SVG in
the `.dc.html` before shipping it. Copying a file is not porting it.

### D-098: PostCSS is pinned empty so a neighbour's Tailwind cannot leak in

The first production build printed a Tailwind warning about a missing
`content` option. Nothing in `web/` depends on Tailwind. Vite resolves PostCSS
configuration by walking up the directory tree, and two levels above this
repository sits an unrelated project with a `postcss.config.js` that loads
Tailwind and autoprefixer. The build was picking it up, which would have
dropped Tailwind's preflight reset underneath a design ported as verbatim
inline styles.

An empty inline `css.postcss` in the Vite config stops the search. The
declaration is the point as much as the fix: this application has no CSS
framework, and now it cannot acquire one by being in the wrong directory.

### D-099: landing prose is frozen, landing state words are not

The brief says the design wins on layout, typography and prose, and the
honesty gate wins on any string that asserts a system state. Both apply to the
landing page and they land in different places.

The Redis to Postgres story, the revision table, the FAQ, the SDK snippets and
the terminal transcript are marketing prose explaining the product to someone
who has never signed in. They are frozen and they ship character for
character, including the sample workspace names inside them.

The connector scene is different. It prints `CONNECTED` next to GitHub and
Slack and `SYNCING` next to Notion, directly under a line that reads STATES
ARE HONEST. There is no connector implementation anywhere under `src/`: a
search for github or oauth returns nothing. So those three words are claims
the product cannot support, and the design's own line is the specification
that forbids them. Every named connector reads `PLANNED`, and Custom ingestion
reads `AVAILABLE` because the ingest pipeline genuinely runs. Layout,
grouping, coordinates, icons and dot treatment are untouched.

The states live in one file, `web/src/design/connectors.ts`, so the landing
catalogue and the application's connector list cannot drift apart, and a
connector that becomes real changes in one place.

### D-100: the terminal block keeps its state words

The CLI section renders a drawn terminal containing a connected HydraDB line,
a model line and a status bar, and the design prints CONCEPT PREVIEW directly
beneath it. That label is what makes those words legitimate: they are contents
of an illustration, in the same category as the box drawing characters around
them, and nothing presents them as a reading of the visitor's own system. They
ship verbatim.

The two rules inside the box are written as repeats of the box drawing
character, thirty-six and forty-five, taken by counting the design's own
characters. Identical output, and the count is checkable instead of being
forty-five characters somebody has to trust.

### D-101: where `Lacuna.html` and the design file disagree, the design file wins

The two are meant to be the same page, and across twenty-eight scenes they
measure identically: same scene order, same heights, same font sizes,
letter-spacings, line-heights and colours, same total document height of
40280px at 1280x800. One scene differs. The architecture scene in the design
file carries a second `data-mhide` block along the bottom, twelve capability
labels and a HydraDB line, and the compiled oracle has no such block.

`Lacuna.html` was compiled before the design file was last edited. The brief
names the design file as the only source of truth and says that if something
is not in it, it does not exist; it does not say the reverse. The block is in
the design file, so it is in the port, and the oracle is stale in that one
spot.
