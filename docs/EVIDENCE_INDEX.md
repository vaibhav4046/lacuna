...
| cross-runtime auth/voice failure mapping, ready preview | commit `502393b` maps provider timeout, microphone permission and autoplay failures by stable error name across browser/runtime realms; 2,261/2,261 tests pass and the Vercel preview is READY, but the stable alias remains on the secret-bearing production artifact until a quota-free promotion is available | `src/auth/google.ts`, `web/src/voice/browser.ts`, `web/src/voice/playback.ts`, `tests/unit/google-auth.test.ts`, `tests/unit/voice-browser.test.ts`, preview `dpl_6ZAqSA6UipwfqkxXFhWvv2DMpfds` |
