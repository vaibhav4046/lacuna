...
| Browser console, every route | Console errors, uncaught exceptions, failed and 400-and-above requests, horizontal overflow and whether anything was drawn, at nine viewports from 360px to 4K, then again with prefers-reduced-motion set | `npm run audit:routes` and `npm run audit:routes -- <url> --reduced-motion` | 198 of 198 in each pass. 0 console errors, 0 exceptions, 0 failed requests, 0 routes scrolling sideways. The first run of this audit failed 18 of 23: every signed-in route scrolled sideways on a phone, which is fixed | [artifacts/route-audit/routes.json](../artifacts/route-audit/routes.json), [routes-reduced-motion.json](../artifacts/route-audit/routes-reduced-motion.json), [scripts/route-audit.ts](../scripts/route-audit.ts) |
...
| ChatGPT custom app | Production public MCP health, ask, timeline, explain, sentence read, search and fetch against HydraDB Cloud | invoked through the installed ChatGPT Lacuna app | `VERIFIED READ ONLY`; seven of seven public tools accepted | [artifacts/verification/2026-08-21-v10/chatgpt-public-connector.json](../artifacts/verification/2026-08-21-v10/chatgpt-public-connector.json) |
...
| Local node, counts | The loaded graph reported back by the CLI | `npm run cli -- status` | Session 72, Message 5246, EvidenceSpan 174, Claim 174, Entity 86 | [RELEASE_GATE.md](../RELEASE_GATE.md) records the ingest as 5752 vertices and 5908 edges; the per-label counts are the output of this session's run |
...
| Unit tests | The suite that needs no database | `npx vitest run tests/unit` | 1,208 passed, 59 files | [RELEASE_GATE.md](../RELEASE_GATE.md), recorded in [artifacts/verification/2026-08-19/unit.txt](../artifacts/verification/2026-08-19/unit.txt). Earlier dated artifacts keep their own counts and are not edited |
...
| Under load | 400 requests at concurrency 12 against the deployed ask endpoint, every response compared to the same question asked alone | `npm run soak` | 26.3 a second, p50 387ms, p95 805ms, p99 1423ms, 0 failures, every answer unchanged | [artifacts/soak/soak.json](../artifacts/soak/soak.json), [scripts/soak.ts](../scripts/soak.ts) |
| Blast radius, both stores | A package change walked for what it reaches, on HydraDB Cloud and on the node | `npm run proof` | 13 services at depth 3 for both `pact-check` and `moss-index`, the two stores returning the same set, one superseded dependency edge refused | [artifacts/proof/proofs.json](../artifacts/proof/proofs.json) |
| False premise, three clients | An unsupported premise beside a real revision and an unknown subject, through the deployed web, the CLI and an MCP subprocess | `npm run proof` | `THREE_CLIENTS_IDENTICAL: true`, 1 answered and 2 refused with different reason codes, none guessed | [artifacts/proof/proofs.json](../artifacts/proof/proofs.json) |
| Mobile and reduced motion | Nine viewports from 360x800 to 3840x2160, then the same sweep with prefers-reduced-motion set | `npm run audit:routes`, then with `--reduced-motion` | 198 of 198 in each pass, 0 scrolling sideways | [artifacts/route-audit/routes.json](../artifacts/route-audit/routes.json), [routes-reduced-motion.json](../artifacts/route-audit/routes-reduced-motion.json) |
...
| Public copy | Landing, app routes, onboarding, auth, README and the social captions, for filler, em dashes as punctuation, exclamation marks and over-long sentences | `npm run copy:lint` | 47 files, 0 findings | [scripts/copy-lint.ts](../scripts/copy-lint.ts) |
| Secrets | Every live key value searched across tracked files and the whole history | `git grep -F`, `git log --all -S` | 0 tracked files, 0 commits | [docs/CREDENTIAL_ROTATION_CHECKLIST.md](CREDENTIAL_ROTATION_CHECKLIST.md) |
| Architecture boundaries | The web importing the resolver, a surface opening its own store, a client re-deciding which claim is current, a route naming a gold answer | `npx vitest run tests/unit/architecture.test.ts` | 8 of 8, and each guard verified by deliberately breaking it | [tests/unit/architecture.test.ts](../tests/unit/architecture.test.ts), [docs/ARCHITECTURE_INVARIANTS.md](ARCHITECTURE_INVARIANTS.md) |
...
- **Roughly a fifth of the specified harness exists.** Counted item by item in
  ...
  present is scoped to a single question.
