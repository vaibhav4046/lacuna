    ...
    <section data-scene="contra" style={{ position: 'relative', height: '190vh' }}>
      <div style={{ position: 'sticky', top: 0, height: '100vh', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 'max(13%, 96px)', left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '18px', textAlign: 'center', padding: '0 20px' }}>
          ...
          <button className="hv-text" data-fx="contra-x" onClick={() => go('/demo/graph')} style={{ opacity: 0, background: 'none', border: 'none', cursor: 'pointer', fontFamily: MONO, fontSize: '11px', fontWeight: 500, letterSpacing: '0.2em', color: '#BDBDBD', padding: '8px 2px', borderBottom: '1px solid rgba(255,255,255,0.25)' }}>INSPECT CONFLICT</button>
        </div>
      </div>
    </section>
  );
