...
| --- | --- | --- |
| focused connector, auth, voice, agent and onboarding gates | focused connector/auth/voice/agent regressions plus onboarding readiness coverage passed; current candidate full unit suite is 2,266/2,266 | serial terminal runs on 2026-08-22 with one worker |
...
| anonymous public agent creation, production | both explore/demo POST names return `403 public_preview_read_only`; invalid JSON on the legacy alias is refused before body/provider processing | deployment `dpl_32sKxNhyJLFUYwdHyCY2JJF2srkz`, `src/api/router.ts`, `tests/unit/demo-api.test.ts` |
...
| stable agent predicate coverage gap | stable production still completed the real `production and` recommendation with `0 CLAIMS`; the fix to include extracted `storage`/`policy` properties is committed in `f052213` and ready in preview `dpl_FXC76ccc5PN6XVpuAmGrPj3tYxDG`, but cannot be promoted while Vercel reports `api-deployments-free-per-day` | live Agents browser run on 2026-08-22, `api/index.ts`, `src/extract/extract.ts`, preview deployment `lacuna-5g2fh7r07` |
...
| public connector catalogue, production | 31/31 demo smoke gates; eight redacted entries, all eight `available`, no private observations | `GET https://lacuna-five.vercel.app/api/explore/connectors`, `scripts/smoke-demo.ts` |
...
| OAuth referrer suppression, ready branch | auth redirects now send `Referrer-Policy: no-referrer`, preventing callback code/state URLs from becoming browser referrers; Google HTTP boundary 13/13 and full unit suite 2,266/2,266 pass; production promotion remains quota-blocked | `src/api/router.ts`, `tests/unit/google-auth-api.test.ts`, commit `7fa2000`, branch `v10-zeus-release` |
| first-run private memory readiness, ready branch | onboarding retries only while its accepted memory receipt reports indexing pending, avoiding a false no-answer immediately after a successful write; regression 3/3 and full unit suite 2,266/2,266 pass; preview creation is quota-blocked | `web/src/onboarding/Onboarding.tsx`, `web/src/onboarding/readiness.ts`, `tests/unit/onboarding-readiness.test.ts`, commit `8618a45` |
| cross-runtime auth/voice failure mapping, ready preview | commits `502393b` and `eaba72c` map provider, microphone, autoplay and browser-fetch timeout failures by stable error name across runtime realms; 2,263/2,263 tests pass and the Vercel preview is READY, but the stable alias remains on the secret-bearing production artifact until a quota-free promotion is available | `src/auth/google.ts`, `web/src/api/client.ts`, `web/src/voice/browser.ts`, `web/src/voice/playback.ts`, `tests/unit/google-auth.test.ts`, `tests/unit/web-auth-client.test.ts`, `tests/unit/voice-browser.test.ts`, preview `dpl_6ZAqSA6UipwfqkxXFhWvv2DMpfds` |
| read-only voice binding-race fallback, ready preview | commit `4cee9b1` allows typed/read-only questions to use the authenticated direct-read path when the optional planner loses a session-binding race; writes and navigation remain fail-closed; focused and full tests pass, but stable promotion is quota-blocked | `web/src/voice/assistant-controller.ts`, `tests/unit/voice-assistant-controller.test.ts`, preview `dpl_FXC76ccc5PN6XVpuAmGrPj3tYxDG` |
| accepted connector documents with pending indexing, ready preview | commit `6756da4` keeps the exact receipt counts while explicitly distinguishing stored accepted documents from unconfirmed search indexing; 2,262/2,262 tests pass and the branch preview is READY, but the stable alias remains on the secret-bearing production artifact until a quota-free promotion is available | `web/src/app/routes/connectors.tsx`, `tests/unit/web-connectors.test.ts`, preview branch deployment `lacuna-dkok8vs7o` |
...
| cross-browser inline voice playback, production code path | blob-backed playback sets inline/preload hints and retries suspended Web Audio before optional analyser attachment; native audio remains the contract when metering is unavailable | `web/src/voice/playback.ts`, `tests/unit/voice-browser.test.ts`, production deployment `dpl_32sKxNhyJLFUYwdHyCY2JJF2srkz` |
