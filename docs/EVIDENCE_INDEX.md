...
| provider-backed voice boundary, production | 7/7 voice smoke gates; real ElevenLabs single-use token and `audio/mpeg` response (25,958 bytes in this run), bounded without printing provider secrets | `scripts/smoke-voice.ts`, production deployment `dpl_9oRBwJsQLRKpBLBN5BtURxrAGFJe` |
...
| clean-browser auth submit guard, production code path | browser auth mutations prime a CSRF cookie with a bounded read-only session preflight before the first credential submit; if the cookie cannot be established, the server still refuses the mutation | `web/src/api/client.ts`, `tests/unit/web-auth-client.test.ts`, production deployment `dpl_9oRBwJsQLRKpBLBN5BtURxrAGFJe` |
