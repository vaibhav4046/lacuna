...
/**
 * Some embedded browsers expose Web Crypto but not randomUUID. Voice intent
 * requests still need a canonical v4 id, so use getRandomValues when the
 * convenience method is absent and keep the final fallback non-authoritative.
 */
...
export class VoiceOperationExecutor {
  ...
  constructor(options: VoiceOperationExecutorOptions = {}) {
    ...
    this.#randomUUID = options.randomUUID ?? createVoiceRequestId;
