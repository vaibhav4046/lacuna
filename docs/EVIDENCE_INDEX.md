...
| --- | --- | --- |
| focused connector, auth, voice and agent gates | 329 focused connector tests plus private-mutation/auth/voice/agent regressions passed; current candidate full unit suite 2,258/2,258 (115 files) | serial terminal runs on 2026-08-22 with one worker |
...
| anonymous public agent creation, production | both explore/demo POST names return `403 public_preview_read_only`; invalid JSON on the legacy alias is refused before body/provider processing | deployment `dpl_Bdod4AG9yLCqiTnESEnxetQvJ4Ch`, `src/api/router.ts`, `tests/unit/demo-api.test.ts` |
...
| Google callback stall/parameter guard (queued) | release candidate rejects malformed or oversized state/code before provider contact; token/JWKS exchange now reads bounded JSON and cancels a headers-only body at the ten-second deadline instead of freezing sign-in; current branch preview lacks the production Hydra context store and is not promoted because the Vercel quota is exhausted | `src/api/router.ts`, `src/auth/google.ts`, `web/src/auth/google-problem.ts`, `tests/unit/google-auth.test.ts`, `tests/unit/google-auth-api.test.ts`, preview `dpl_EeU5TvrryEDZ3CLGsSNv9ZBDCh8y` |
| landing particle/text collision guard (queued) | candidate marks architecture, model-routing and voice overlays as canvas shields, preventing bright particle dots from rendering over live labels; static contract regression passes in the 2,258-test suite | `web/src/landing/Arch.tsx`, `web/src/landing/Route.tsx`, `web/src/landing/Voice.tsx`, `web/src/canvas/engine.ts`, `tests/unit/web-product-contracts.test.ts`, candidate branch (not production-promoted) |
| private session guard recovery (queued) | candidate renders a bounded checking state and visible retry action when `/api/session` fails, so a transient auth/network error cannot leave a blank frozen private route; static contract regression passes in the 2,258-test suite | `web/src/app/RequireSession.tsx`, `web/src/api/session.tsx`, `tests/unit/web-product-contracts.test.ts`, candidate branch (not production-promoted) |
| failed panel recovery (queued) | candidate adds a keyboard-sized `Try again` action to failed data panels so Agents, Work, Context and connector reads do not end at a dead-end error message; static contract regression passes in the 2,258-test suite | `web/src/app/state.tsx`, `tests/unit/web-product-contracts.test.ts`, candidate branch (not production-promoted) |
...
| serverless connector event-method guard (queued) | Candidate request/response lifecycle hooks treat `.once`, `.off`, and `.removeListener` as optional adapter methods, preventing a valid HTTPS import from becoming a TypeError/500; focused connector and lifecycle regressions pass | `src/api/request-lifecycle.ts`, `src/api/router.ts`, `tests/unit/request-lifecycle.test.ts`, `tests/unit/connectors-api.test.ts`, candidate branch (not production-promoted) |
| stalled speech-body cancellation (queued) | Candidate cancels the response body reader when speech headers arrive but the provider stream stalls, returning a bounded provider failure instead of leaving voice busy; regression passes in the 2,258-test suite | `web/src/voice/browser.ts`, `tests/unit/voice-browser.test.ts`, candidate branch (not production-promoted) |
| stalled voice JSON-body cancellation (queued) | Candidate cancels the JSON response reader when voice query headers arrive but the provider body stalls, returning a bounded provider failure instead of leaving voice busy; regression passes in the 2,258-test suite | `web/src/voice/browser.ts`, `tests/unit/voice-browser.test.ts`, candidate branch (not production-promoted) |
| GitHub body cancellation (queued) | Candidate cancels the default fetch transport's response body reader when its bounded importer signal fires after headers arrive, returning `github_timeout` instead of hanging; regression passes in the 2,258-test suite | `src/connectors/github.ts`, `tests/unit/connectors-github.test.ts`, candidate branch (not production-promoted) |
| HydraDB body cancellation (queued) | Candidate cancels the HydraDB response body reader when a caller aborts after headers arrive, returning a bounded transport failure instead of hanging; regression passes in the 2,258-test suite | `src/hydra/client.ts`, `tests/unit/client.test.ts`, candidate branch (not production-promoted) |
| browser connector response-body cancellation (queued) | Candidate reads connector JSON through a bounded stream and cancels/releases the reader when caller abort or deadline fires; regression passes in the 2,258-test suite | `web/src/api/connectors.ts`, `tests/unit/web-connectors-client.test.ts`, candidate branch (not production-promoted) |
| Agents/Work action recovery (queued) | Candidate schedule, run-now, cancel and retry controls clear busy state and show bounded failure copy when requests throw; helper has two focused regressions in the 2,258-test suite | `web/src/app/agent-actions.ts`, `web/src/app/routes/agents.tsx`, `web/src/app/routes/work.tsx`, `tests/unit/agent-actions.test.ts`, candidate branch (not production-promoted) |
| shared browser API response-body cancellation (queued) | Candidate session, workspace and agent reads/mutations cancel/release stalled JSON readers on caller abort or deadline; regression passes in the 2,258-test suite | `web/src/api/client.ts`, `tests/unit/web-auth-client.test.ts`, candidate branch (not production-promoted) |
