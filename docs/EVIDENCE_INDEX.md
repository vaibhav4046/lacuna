...
| latest Git-connected production receipt | GitHub `main` deployment `dpl_GWF5vaPYaRQgNkZhDrfit81KmPkE` owns `https://lacuna-five.vercel.app`; auth 3/3, Google 16/16, provider voice 7/7 and demo/catalogue 32/32 pass after the production environment was bound. | [production-git-deploy-latest.txt](../artifacts/verification/2026-08-22-v10/production-git-deploy-latest.txt) |
...
| legacy native voice playback compatibility, production | commit `1dbc513` handles WebKit-style `Audio.play()` implementations that return no Promise while preserving native `playing`/`ended` lifecycle events; voice browser + assistant tests pass 54/54, root typecheck and web build pass, and the production deployment `dpl_GWF5vaPYaRQgNkZhDrfit81KmPkE` now serves the patch | `web/src/voice/playback.ts`, `tests/unit/voice-browser.test.ts`, deployment `dpl_GWF5vaPYaRQgNkZhDrfit81KmPkE` |
