...
| --- | --- | --- |
| focused connector, auth and voice gates | 166 structured connector tests plus private-mutation/auth/voice regressions passed; full unit suite 2,222/2,222 (111 files) | serial terminal runs on 2026-08-22 with one worker |
...
| provider-backed voice boundary, production | 7/7 voice smoke gates; real ElevenLabs single-use token and `audio/mpeg` response (25,122 bytes in this run), bounded without printing provider secrets | `scripts/smoke-voice.ts`, production deployment `dpl_7PSU1HfS71ZuNyVbCdB6Lbq1cn9d` |
