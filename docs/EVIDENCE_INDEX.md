...
| --- | --- | --- |
| focused connector, auth, voice and agent gates | 166 structured connector tests plus private-mutation/auth/voice/agent regressions passed; current candidate full unit suite 2,245/2,245 (114 files) | serial terminal runs on 2026-08-22 with one worker |
...
| Google callback stall/parameter guard (queued) | release candidate rejects malformed or oversized state/code before provider contact; token/JWKS exchange has a measured ten-second deadline and returns a fixed timeout message instead of freezing the sign-in form; current branch preview lacks the production Hydra context store and is not promoted because the Vercel quota is exhausted | `src/api/router.ts`, `src/auth/google.ts`, `web/src/auth/google-problem.ts`, `tests/unit/google-auth-api.test.ts`, preview `dpl_EdtFRMoQWo5XBcbencCJnepDACpN` |
