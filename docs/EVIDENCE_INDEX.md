...
| owner legacy Google migration gate | A single exact-owner `LACUNA_LEGACY_GOOGLE_MIGRATION_EMAIL` is configured in Vercel Production. The current immutable stable deployment cannot read an environment change until a new deployment is created; promotion is pending the exhausted daily deployment quota. | Vercel Production environment listing and `src/api/router.ts` allowlisted migration path |
