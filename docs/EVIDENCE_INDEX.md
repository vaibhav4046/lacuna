...
| Google callback stall/parameter guard | malformed or oversized state/code is rejected before provider contact; token/JWKS exchange has a measured ten-second deadline and returns a fixed timeout message instead of freezing the sign-in form | `src/api/router.ts`, `src/auth/google.ts`, `web/src/auth/google-problem.ts`, `tests/unit/google-auth-api.test.ts` |
