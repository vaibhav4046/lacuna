...
voice boundary 7/7. A historical candidate run passed 2,294/2,294 unit tests;
the current candidate's focused auth, connector, agent, voice and ingestion
suites are green, while a serial all-unit run still exposes a Vitest worker
isolation failure around the Node worker-thread file-parser tests. The
production web build and route matrix are green. ChatGPT has called all seven public MCP
