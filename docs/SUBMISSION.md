...
1,202 unit tests across 59 files run with no database. Four contract suites run
every query builder against a live HydraDB node and fail loudly if the node is
absent rather than quietly mocking it. 1,279 tests in total with a node
running.
```
...
directly, vitest for the 1,279 tests, and @huggingface/transformers.
